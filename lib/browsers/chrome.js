...
chrome.loadUrlThenWaitForPageLoadEvent = function(tab, url) {
	return new Promise((resolve, reject) => {
		...
		.then(() => {
			...
			util.log(`Page ${url} enabled, waiting for prerenderReady flag... Checking interval: ${pageDoneCheckInterval}, timeout: ${pageLoadTimeout}`);

			var checkIfDone = () => {
				if (finished) {
					util.log('############################################################')
					util.log('Done Loadding!!')
					util.log('############################################################')
					return;
				}
        ...
        util.log('############################################################')
        util.log('Checking if page is done loading....')
        util.log('############################################################')
				this.checkIfPageIsDoneLoading(tab).then((doneLoading) => {
					...
					if (!doneLoading && !finished) {
            util.log('############################################################')
            util.log('Waiting For prerenderReady, Retrying!!')
            util.log('############################################################')
...
chrome.checkIfPageIsDoneLoading = function(tab) {
	return new Promise((resolve, reject) => {
		tab.Runtime.evaluate({
			...
			let shouldWaitForPrerenderReady = !tab.prerender.url.match(/\.(gif|jpg|jpeg|png|js|css|json|webp|map)/i) || (typeof prerenderReady === 'boolean');
			let waitAfterLastRequest = tab.prerender.waitAfterLastRequest || this.options.waitAfterLastRequest;

			let doneLoading = tab.prerender.numRequestsInFlight <= 0 && tab.prerender.lastRequestReceivedAt < ((new Date()).getTime() - waitAfterLastRequest)

      // util.log('############################################################')
      // util.log(`PrerenderReady => ${(!shouldWaitForPrerenderReady && doneLoading) || (shouldWaitForPrerenderReady && doneLoading && prerenderReady)}`)
      // util.log(`Done Loading => ${doneLoading}`)
      // util.log('############################################################')
      util.log(`Checked for prerenderReady, got: ${prerenderReady}. Done loading: ${doneLoading}, current request count: ${tab.prerender.numRequestsInFlight}`);

			// resolve((!shouldWaitForPrerenderReady && doneLoading) || (shouldWaitForPrerenderReady && prerenderReady));
			if (shouldWaitForPrerenderReady) {
				resolve(prerenderReady); // simplify prerender-ready logic
			} else {
				resolve(doneLoading);
			}
		}).catch((err) => {
