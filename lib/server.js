    ...
    this.firePluginEvent('requestReceived', req, res)
        .then(() => {
            if (req.prerender.redisKey) {
                getAsync(req.prerender.redisKey).then((res) => {
                    if (res && req.prerender.render_param !== 'clear') {
                        req.prerender.content = res;
                        throw 'Key Founded';
                    }
                    return Promise.resolve();
                }).then(() => {
                    util.log('*** PRERENDERING ***');
                    return this.renderPage(req, res);
                }).then(() => {
                    util.log('*** Serving rendered key ***');
                    req.prerender.statusCode = 200;
                    req.prerender.prerenderData = {};
                    this.finish(req, res);
                }).catch(() => {
                    //this.renderPage();
                    util.log("*** Serving cached key ***");
                    req.prerender.statusCode = 200;
                    req.prerender.prerenderData = {};
                    this.finish(req, res);
                });
...
server.validBody = function (body) {
    return !!(body && body !== '<html><head></head><body></body></html>' && body.length > 50);
};

server.renderPage = function (req, res) {
    ...
    return this.waitForBrowserToConnect().then(() => {
        ...
        switch (req.prerender.renderType) {
            case 'png':
