...
server.onRequest = function(req, res) {
	  ...
	  this.firePluginEvent('requestReceived', req, res)
        .then(() => {
            if(req.prerender.redisKey){
                getAsync(req.prerender.redisKey)
                    .then((res) => {
                        if(!res){
                            util.log('*** Redis Error: Getting key! ***', res);
                            return Promise.resolve(); // Promise resolved to continue to next promise
                        }
		                    req.prerender.content = res;
                        return Promise.reject(); // Promise rejected to stop and response
                    }).then(() => {
                        util.log('*** PRERENDERING ASYNC!! ***');
                        ...
                        this.waitForBrowserToConnect().then(async () => {
                            ...
                            if(this.validBody(req.prerender.content)) {
                                util.log('*** Saving redis key ***', req.prerender.redisKey);
                                client.set(req.prerender.redisKey, req.prerender.content, redis.print);
                            }
                            else{
                                throw 'NotValidBodyError, check if host to rendering is upstart', req.prerender.content;
                            }
                        }).catch((error) => {
                            util.log('Rendering Error: ', error);
                        });

                        return Promise.reject();
                    }).catch((error) => {
                        util.log('*** FINISH!! ***');
                        if (!req.prerender.content) req.prerender.content = {rendering: true};
                        req.prerender.statusCode = 200;
		                    req.prerender.prerenderData = {};
                        this.finish(req, res);
                    });
            }
        }).catch((err) => {
...
server.validBody = function(body) {
    debugger;
    if (body && body !== '<html><head></head><body></body></html>' && body.length > 50) {
        return true;
    }
    return false;
};
